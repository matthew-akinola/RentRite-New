...
const Form = () => {
   ...
   const handleChange = (event:any)=>{
    if (uploadNameRef.current){
      uploadNameRef.current.value = event.target.files[0].name
    }
  ...
  return (
    <form className='py-10 bg-white px-8' encType="multipart/form-data">
      ...
      <div className='my-10'>
        ...
        <select name="" id="blog-tittle" className='w-full border-[1.5px] border-[#D0D0D5] rounded-lg h-16 px-3 py-4 bg-[#FBFBFC]' >
        ...
        <div className='my-10'>
            ...
            <div onClick={handleUpload} className='w-full border-[1.5px] border-[#D0D0D5] rounded-lg h-16 px-3 py-4 bg-[#FBFBFC] flex flex-row' >
              ...
              <input type="file" name="image-upload" id="image-upload"  accept='image/*' className='hidden' ref={uploadImageRef} onChange={handleChange} />
              <input type="text" name="" id="" ref={uploadNameRef} disabled className='px-2 w-[50%] sm:w-fit bg-inherit text-[#B1B0B9]' placeholder='Upload an image' />
        ...
        <label htmlFor="content-description" className='block text-sm font-bold text-[#2B2A30] mb-2 uppercase'>Content Description</label>
            <textarea className='h-[459px] resize-none border-[1.5px] border-[#D0D0D5] rounded-lg w-full bg-[#FBFBFC] px-3 py-4' name="content-description" id="content-description" placeholder='Start Writing content' ></textarea>
        </div>
        <div className='my-10 flex'>

            <div>
                <input type="checkbox" name="featured-post" id="featured-post" className='rounded-lg border border-[#2B2A30]' />
